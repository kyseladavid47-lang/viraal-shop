...
// Track sent emails to prevent duplicates on page refresh
  ...
  try {
    ...
    if (customerEmail && !emailsSent.has(session_id)) {
      ...
      await transporter.sendMail({
        from: '"VIRAAL" <viraal-shop@protonmail.com>',
        to: customerEmail,
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:540px;margin:0 auto;padding:48px 40px;background:white;">
            <div style="font-size:26px;font-weight:900;margin-bottom:32px;">
              VI<span style="color:#FF6B35;">R</span>AAL
            </div>
            <div style="background:#DCFCE7;color:#16A34A;font-size:13px;font-weight:700;padding:6px 14px;display:inline-block;margin-bottom:24px;">
              ✓ Payment Confirmed
            </div>
            <h1 style="font-size:24px;color:#0D0D0D;margin:0 0 12px;">
